...
const formNuevoContactoEl = document.getElementById("formNuevoContacto");
const modalNuevoContactoEl = document.getElementById("modalNuevoContacto");

formNuevoContactoEl.addEventListener("submit", function(event){
    event.preventDefault();

    //CAPTURAMOS LA DATA DEL FORMULARIO
    let dataFormulario = new FormData(formNuevoContactoEl);

    let nombre = dataFormulario.get("nombre");
    let cbu = dataFormulario.get("cbu");
    let alias = dataFormulario.get("alias");
    let banco = dataFormulario.get("banco");
    ...
    //CERRAMOS EL MODAL
    let modal = bootstrap.Modal.getInstance(modalNuevoContactoEl);
    modal.hide();
